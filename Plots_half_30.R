...
load("~/Desktop/Work/Projects/HBGM/RData/La_Liga/comparison_liga_half_30.RData")
load("~/Desktop/Work/Projects/HBGM/RData/Premier_League/comparison_premier_half_30.RData")
load("~/Desktop/Work/Projects/HBGM/RData/Bundesliga/comparison_bundes_half_27.RData")
# Merging data


comparison_bundes_half_27$metrics$league <- "Bundesliga"
comparison_liga_half_30$metrics$league <- "La Liga"
comparison_premier_half_30$metrics$league <- "Premier League"

bundesliga <-  comparison_bundes_half_27$metrics %>%
  mutate(
    across(
      where(is.numeric),
      ~ round(., 3)
...
liga <- comparison_liga_half_30$metrics %>%
  mutate(
    across(
      where(is.numeric),
      ~ round(., 3)
...
premier <- comparison_premier_half_30$metrics %>%
  mutate(
    across(
      where(is.numeric),
      ~ round(., 3)
...
ggsave(
  filename = "plot_comp_half_30.pdf", path = "Plots",
  plot = plot_comp,
...
purrr::iwalk(plots_by_league, ~ {
  ggsave(filename = paste0("plot_half_30", gsub(" ", "_", .y), ".pdf"),
         path = "Plots",
         plot     = .x,
         width = 17, height = 10, device = "pdf", dpi = 500, useDingbats = FALSE)
})
...
load("~/Desktop/Work/Projects/HBGM/RData/Bundesliga/time_results_bundes_half_27.RData")    
load("~/Desktop/Work/Projects/HBGM/RData/La_Liga/time_results_liga_half_30.RData") 
load("~/Desktop/Work/Projects/HBGM/RData/Premier_League/time_results_premier_half_30.RData")

df_bundes <- time_results_bundes_half_27    %>% mutate(league = "Bundesliga")
df_liga   <- time_results_liga_half_30     %>% mutate(league = "La Liga")
df_epl    <- time_results_premier_half_30  %>% mutate(league = "EPL")
...
comp_time_half_30 <- ggplot(df_all, aes(x = model_type, y = elapsed, fill = model_type)) +
...
ggsave("comp_time_half_30.pdf",
       plot = comp_time_half_30, path = "Plots",
