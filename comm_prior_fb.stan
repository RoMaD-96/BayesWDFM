data{
    ...
    transformed data {
      real lognc_spike = normal_lccdf(0 | 200, 0.1); // \Phi(mu_spike / sigma_spike)
      real lognc_slab  = normal_lccdf(0 | 0, 10); // \Phi(mu_slab / sigma_slab)
    }
    ...
    parameters{
      ...
      real gamma;
      vector<lower=0>[ntimes] comm_prec_att;  // commensurability parameter
      vector<lower=0>[ntimes] comm_prec_def;  // commensurability parameter
    ...
    transformed parameters{
      ...
      vector[ntimes] comm_sd_att = inv_sqrt(comm_prec_att);   // sd for commensurate prior
      vector[ntimes] comm_sd_def = inv_sqrt(comm_prec_def);   // sd for commensurate prior
    ...
    model{
      ...
      for (i in 1:ntimes) {
        target += log_mix(
          0.2
          , normal_lpdf(comm_prec_att[i] | 200, 0.1) - lognc_spike
          , normal_lpdf(comm_prec_att[i] | 0, 10) - lognc_slab
        );

          target += log_mix(
            0.2
            , normal_lpdf(comm_prec_def[i] | 200, 0.1) - lognc_spike
            , normal_lpdf(comm_prec_def[i] | 0, 10) - lognc_slab
         );
      }
      ...
      for (h in 1:(nteams)){
        for (i in 1:ntimes){
          if (prior_dist_num == 1 ){
          att_raw[i,h]~normal(mu_att[i,h], comm_sd_att[i]);
          def_raw[i,h]~normal(mu_def[i,h], comm_sd_def[i]);
        }
        else if (prior_dist_num == 2 ){
          att_raw[i,h]~student_t(hyper_df, mu_att[i,h], comm_sd_att[i]);
          def_raw[i,h]~student_t(hyper_df, mu_def[i,h], comm_sd_def[i]);
        }
        else if (prior_dist_num == 3 ){
          att_raw[,h]~student_t(1, mu_att[i,h], comm_sd_att[i]);
          def_raw[,h]~student_t(1, mu_def[i,h], comm_sd_def[i]);
        }
      }
    }
