...
model_runners <- list(
  ...
  biv_pois_comm      = function(seed) stan_foot(data = premier, model = "biv_pois", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed),
  diag_biv_pois_comm = function(seed) stan_foot(data = premier, model = "diag_infl_biv_pois", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed),
  double_pois_comm   = function(seed) stan_foot(data = premier, model = "double_pois", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed),
  neg_bin_comm       = function(seed) stan_foot(data = premier, model = "neg_bin", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed),
  skellam_comm       = function(seed) stan_foot(data = premier, model = "skellam", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed),
  zero_skellam_comm  = function(seed) stan_foot(data = premier, model = "zero_infl_skellam", predict = 30, dynamic_type = "seasonal", dynamic_weight = TRUE, dynamic_par = list(spike_prob = 0.01, slab = normal(0, 5), spike = normal(100, 0.1)), home_effect = TRUE, iter_sampling = 1000, chains = 4, parallel_chains = 4, method = "MCMC", seed = seed)
)



time_results_premier_half_30 <- tibble(model = character(), run = integer(), elapsed = double())
base_seed <- 1000L
...
for (model_name in names(model_runners)) {
  ...
  time_results_premier_half_30 <- bind_rows(
    time_results_premier_half_30,
    tibble(
      model = model_name,
      run = seq_along(times),
      elapsed = times
...
save(time_results_premier_half_30, file = "RData/Premier_League/time_results_premier_half_30.RData")
