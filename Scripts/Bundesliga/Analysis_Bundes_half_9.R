...
comparison_bundes_half_9 <- compare_foot(source = list(
  biv_pois_comm = biv_pois_comm,
...
comparison_bundes_half_9

save(comparison_bundes_half_9, file = "RData/Bundesliga/comparison_bundes_half_9.RData")
...
get_ability_df <- function(stan_obj, variant_name, teams, type = "both") {
  ...
  all_teams <- unique(c(bundesliga$home_team, bundesliga$away_team))
...
foot_abilities(biv_pois_comm, bundesliga)
foot_abilities(biv_pois_owen, bundesliga)


my_teams <- c("Bayern Munich", "Hoffenheim")

df_comm <- get_ability_df(biv_pois_comm, "Comm", my_teams)
df_owen <- get_ability_df(biv_pois_owen, "Owen", my_teams)
df_egidi <- get_ability_df(biv_pois, "Egidi", my_teams)

df_abilities_bundesliga_half_10 <- bind_rows(df_comm, df_owen, df_egidi)
save(df_abilities_bundesliga_half_10, file = "RData/Bundesliga/Abilities/df_abilities_bundesliga_half_10.RData")
...
save(stats_bundes, file = "RData/Bundesliga/stats_bundes_half_9.RData")
View(stats_bundes)
